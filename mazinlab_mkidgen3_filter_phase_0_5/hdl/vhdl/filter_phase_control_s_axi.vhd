...
entity filter_phase_control_s_axi is
generic (
...
end entity filter_phase_control_s_axi;
...
architecture behave of filter_phase_control_s_axi is
    ...
    constant ADDR_KEEP_CTRL      : INTEGER := 16#20#;
    constant ADDR_LASTGRP_DATA_0 : INTEGER := 16#24#;
    constant ADDR_LASTGRP_CTRL   : INTEGER := 16#28#;
    ...
    -- internal registers
    signal int_keep            : UNSIGNED(127 downto 0) := (others => '0');
    signal int_lastgrp         : UNSIGNED(6 downto 0) := (others => '0');
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_LASTGRP_DATA_0 =>
                        rdata_data <= RESIZE(int_lastgrp(6 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_LASTGRP_DATA_0) then
                    int_lastgrp(6 downto 0) <= (UNSIGNED(WDATA(6 downto 0)) and wmask(6 downto 0)) or ((not wmask(6 downto 0)) and int_lastgrp(6 downto 0));
                end if;
            end if;
        end if;
    end process;
