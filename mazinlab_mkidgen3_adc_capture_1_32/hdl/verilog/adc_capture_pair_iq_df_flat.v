// ==============================================================
// RTL generated by Vitis HLS - High-Level Synthesis from C, C++ and OpenCL v2021.1 (64-bit)
// Version: 2021.1
// Copyright (C) Copyright 1986-2021 Xilinx, Inc. All Rights Reserved.
// 
// ===========================================================

`timescale 1 ns / 1 ps 

module adc_capture_pair_iq_df_flat (
        ap_clk,
        ap_rst,
        ap_start,
        ap_done,
        ap_continue,
        ap_idle,
        ap_ready,
        istream_V_TDATA,
        istream_V_TVALID,
        istream_V_TREADY,
        qstream_V_TDATA,
        qstream_V_TVALID,
        qstream_V_TREADY,
        capturesize,
        iq_in8_din,
        iq_in8_full_n,
        iq_in8_write,
        capturesize_c_din,
        capturesize_c_full_n,
        capturesize_c_write
);

parameter    ap_ST_fsm_state1 = 4'd1;
parameter    ap_ST_fsm_state2 = 4'd2;
parameter    ap_ST_fsm_state3 = 4'd4;
parameter    ap_ST_fsm_state4 = 4'd8;

input   ap_clk;
input   ap_rst;
input   ap_start;
output   ap_done;
input   ap_continue;
output   ap_idle;
output   ap_ready;
input  [127:0] istream_V_TDATA;
input   istream_V_TVALID;
output   istream_V_TREADY;
input  [127:0] qstream_V_TDATA;
input   qstream_V_TVALID;
output   qstream_V_TREADY;
input  [26:0] capturesize;
output  [255:0] iq_in8_din;
input   iq_in8_full_n;
output   iq_in8_write;
output  [26:0] capturesize_c_din;
input   capturesize_c_full_n;
output   capturesize_c_write;

reg ap_done;
reg ap_idle;
reg ap_ready;
reg iq_in8_write;
reg capturesize_c_write;

reg    ap_done_reg;
(* fsm_encoding = "none" *) reg   [3:0] ap_CS_fsm;
wire    ap_CS_fsm_state1;
reg   [26:0] capturesize_0_data_reg;
reg    capturesize_0_vld_reg;
reg    capturesize_0_ack_out;
reg    capturesize_c_blk_n;
wire    ap_CS_fsm_state2;
wire    grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_start;
wire    grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_done;
wire    grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_idle;
wire    grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_ready;
wire    grp_pair_iq_df_flat_Pipeline_read_fu_58_istream_V_TREADY;
wire    grp_pair_iq_df_flat_Pipeline_read_fu_58_qstream_V_TREADY;
wire   [255:0] grp_pair_iq_df_flat_Pipeline_read_fu_58_iq_in8_din;
wire    grp_pair_iq_df_flat_Pipeline_read_fu_58_iq_in8_write;
reg    grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_start_reg;
wire    ap_CS_fsm_state3;
wire    ap_CS_fsm_state4;
reg   [3:0] ap_NS_fsm;
reg    ap_block_state1;
reg    ap_ST_fsm_state1_blk;
reg    ap_ST_fsm_state2_blk;
wire    ap_ST_fsm_state3_blk;
reg    ap_ST_fsm_state4_blk;
wire    regslice_both_istream_V_U_apdone_blk;
wire   [127:0] istream_V_TDATA_int_regslice;
wire    istream_V_TVALID_int_regslice;
reg    istream_V_TREADY_int_regslice;
wire    regslice_both_istream_V_U_ack_in;
wire    regslice_both_qstream_V_U_apdone_blk;
wire   [127:0] qstream_V_TDATA_int_regslice;
wire    qstream_V_TVALID_int_regslice;
reg    qstream_V_TREADY_int_regslice;
wire    regslice_both_qstream_V_U_ack_in;
wire    ap_ce_reg;

// power-on initialization
initial begin
#0 ap_done_reg = 1'b0;
#0 ap_CS_fsm = 4'd1;
#0 capturesize_0_data_reg = 27'd0;
#0 capturesize_0_vld_reg = 1'b0;
#0 grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_start_reg = 1'b0;
end

adc_capture_pair_iq_df_flat_Pipeline_read grp_pair_iq_df_flat_Pipeline_read_fu_58(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .ap_start(grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_start),
    .ap_done(grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_done),
    .ap_idle(grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_idle),
    .ap_ready(grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_ready),
    .capturesize(capturesize_0_data_reg),
    .istream_V_TDATA(istream_V_TDATA_int_regslice),
    .istream_V_TVALID(istream_V_TVALID_int_regslice),
    .istream_V_TREADY(grp_pair_iq_df_flat_Pipeline_read_fu_58_istream_V_TREADY),
    .qstream_V_TDATA(qstream_V_TDATA_int_regslice),
    .qstream_V_TVALID(qstream_V_TVALID_int_regslice),
    .qstream_V_TREADY(grp_pair_iq_df_flat_Pipeline_read_fu_58_qstream_V_TREADY),
    .iq_in8_din(grp_pair_iq_df_flat_Pipeline_read_fu_58_iq_in8_din),
    .iq_in8_full_n(iq_in8_full_n),
    .iq_in8_write(grp_pair_iq_df_flat_Pipeline_read_fu_58_iq_in8_write)
);

adc_capture_regslice_both #(
    .DataWidth( 128 ))
regslice_both_istream_V_U(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .data_in(istream_V_TDATA),
    .vld_in(istream_V_TVALID),
    .ack_in(regslice_both_istream_V_U_ack_in),
    .data_out(istream_V_TDATA_int_regslice),
    .vld_out(istream_V_TVALID_int_regslice),
    .ack_out(istream_V_TREADY_int_regslice),
    .apdone_blk(regslice_both_istream_V_U_apdone_blk)
);

adc_capture_regslice_both #(
    .DataWidth( 128 ))
regslice_both_qstream_V_U(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .data_in(qstream_V_TDATA),
    .vld_in(qstream_V_TVALID),
    .ack_in(regslice_both_qstream_V_U_ack_in),
    .data_out(qstream_V_TDATA_int_regslice),
    .vld_out(qstream_V_TVALID_int_regslice),
    .ack_out(qstream_V_TREADY_int_regslice),
    .apdone_blk(regslice_both_qstream_V_U_apdone_blk)
);

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_CS_fsm <= ap_ST_fsm_state1;
    end else begin
        ap_CS_fsm <= ap_NS_fsm;
    end
end

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_done_reg <= 1'b0;
    end else begin
        if ((ap_continue == 1'b1)) begin
            ap_done_reg <= 1'b0;
        end else if (((grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_done == 1'b1) & (1'b1 == ap_CS_fsm_state4))) begin
            ap_done_reg <= 1'b1;
        end
    end
end

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_start_reg <= 1'b0;
    end else begin
        if ((1'b1 == ap_CS_fsm_state3)) begin
            grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_start_reg <= 1'b1;
        end else if ((grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_ready == 1'b1)) begin
            grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_start_reg <= 1'b0;
        end
    end
end

always @ (posedge ap_clk) begin
    if (((~((ap_start == 1'b0) & (1'b1 == ap_CS_fsm_state1)) & (capturesize_0_ack_out == 1'b1) & (1'b1 == 1'b1) & (capturesize_0_vld_reg == 1'b1)) | (~((ap_start == 1'b0) & (1'b1 == ap_CS_fsm_state1)) & (1'b1 == 1'b1) & (capturesize_0_vld_reg == 1'b0)))) begin
        capturesize_0_data_reg <= capturesize;
    end
end

always @ (*) begin
    if (((ap_start == 1'b0) | (ap_done_reg == 1'b1))) begin
        ap_ST_fsm_state1_blk = 1'b1;
    end else begin
        ap_ST_fsm_state1_blk = 1'b0;
    end
end

always @ (*) begin
    if ((capturesize_c_full_n == 1'b0)) begin
        ap_ST_fsm_state2_blk = 1'b1;
    end else begin
        ap_ST_fsm_state2_blk = 1'b0;
    end
end

assign ap_ST_fsm_state3_blk = 1'b0;

always @ (*) begin
    if ((grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_done == 1'b0)) begin
        ap_ST_fsm_state4_blk = 1'b1;
    end else begin
        ap_ST_fsm_state4_blk = 1'b0;
    end
end

always @ (*) begin
    if (((grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_done == 1'b1) & (1'b1 == ap_CS_fsm_state4))) begin
        ap_done = 1'b1;
    end else begin
        ap_done = ap_done_reg;
    end
end

always @ (*) begin
    if (((ap_start == 1'b0) & (1'b1 == ap_CS_fsm_state1))) begin
        ap_idle = 1'b1;
    end else begin
        ap_idle = 1'b0;
    end
end

always @ (*) begin
    if (((grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_done == 1'b1) & (1'b1 == ap_CS_fsm_state4))) begin
        ap_ready = 1'b1;
    end else begin
        ap_ready = 1'b0;
    end
end

always @ (*) begin
    if ((((grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_done == 1'b1) & (1'b1 == ap_CS_fsm_state4)) | ((capturesize_c_full_n == 1'b1) & (1'b1 == ap_CS_fsm_state2)))) begin
        capturesize_0_ack_out = 1'b1;
    end else begin
        capturesize_0_ack_out = 1'b0;
    end
end

always @ (*) begin
    if ((1'b1 == ap_CS_fsm_state2)) begin
        capturesize_c_blk_n = capturesize_c_full_n;
    end else begin
        capturesize_c_blk_n = 1'b1;
    end
end

always @ (*) begin
    if (((capturesize_c_full_n == 1'b1) & (1'b1 == ap_CS_fsm_state2))) begin
        capturesize_c_write = 1'b1;
    end else begin
        capturesize_c_write = 1'b0;
    end
end

always @ (*) begin
    if ((1'b1 == ap_CS_fsm_state4)) begin
        iq_in8_write = grp_pair_iq_df_flat_Pipeline_read_fu_58_iq_in8_write;
    end else begin
        iq_in8_write = 1'b0;
    end
end

always @ (*) begin
    if ((1'b1 == ap_CS_fsm_state4)) begin
        istream_V_TREADY_int_regslice = grp_pair_iq_df_flat_Pipeline_read_fu_58_istream_V_TREADY;
    end else begin
        istream_V_TREADY_int_regslice = 1'b0;
    end
end

always @ (*) begin
    if ((1'b1 == ap_CS_fsm_state4)) begin
        qstream_V_TREADY_int_regslice = grp_pair_iq_df_flat_Pipeline_read_fu_58_qstream_V_TREADY;
    end else begin
        qstream_V_TREADY_int_regslice = 1'b0;
    end
end

always @ (*) begin
    case (ap_CS_fsm)
        ap_ST_fsm_state1 : begin
            if ((~((ap_start == 1'b0) | (ap_done_reg == 1'b1)) & (1'b1 == ap_CS_fsm_state1))) begin
                ap_NS_fsm = ap_ST_fsm_state2;
            end else begin
                ap_NS_fsm = ap_ST_fsm_state1;
            end
        end
        ap_ST_fsm_state2 : begin
            if (((capturesize_c_full_n == 1'b1) & (1'b1 == ap_CS_fsm_state2))) begin
                ap_NS_fsm = ap_ST_fsm_state3;
            end else begin
                ap_NS_fsm = ap_ST_fsm_state2;
            end
        end
        ap_ST_fsm_state3 : begin
            ap_NS_fsm = ap_ST_fsm_state4;
        end
        ap_ST_fsm_state4 : begin
            if (((grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_done == 1'b1) & (1'b1 == ap_CS_fsm_state4))) begin
                ap_NS_fsm = ap_ST_fsm_state1;
            end else begin
                ap_NS_fsm = ap_ST_fsm_state4;
            end
        end
        default : begin
            ap_NS_fsm = 'bx;
        end
    endcase
end

assign ap_CS_fsm_state1 = ap_CS_fsm[32'd0];

assign ap_CS_fsm_state2 = ap_CS_fsm[32'd1];

assign ap_CS_fsm_state3 = ap_CS_fsm[32'd2];

assign ap_CS_fsm_state4 = ap_CS_fsm[32'd3];

always @ (*) begin
    ap_block_state1 = ((ap_start == 1'b0) | (ap_done_reg == 1'b1));
end

assign capturesize_c_din = capturesize_0_data_reg;

assign grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_start = grp_pair_iq_df_flat_Pipeline_read_fu_58_ap_start_reg;

assign iq_in8_din = grp_pair_iq_df_flat_Pipeline_read_fu_58_iq_in8_din;

assign istream_V_TREADY = regslice_both_istream_V_U_ack_in;

assign qstream_V_TREADY = regslice_both_qstream_V_U_ack_in;

endmodule //adc_capture_pair_iq_df_flat
