...
// ==============================================================
`timescale 1ns/1ps
module phase_capture_control_s_axi
#(parameter
...
localparam
    ...
    ADDR_KEEP_DATA_8              = 7'h30,
    ADDR_KEEP_DATA_9              = 7'h34,
    ADDR_KEEP_DATA_10             = 7'h38,
    ADDR_KEEP_DATA_11             = 7'h3c,
    ADDR_KEEP_DATA_12             = 7'h40,
    ADDR_KEEP_DATA_13             = 7'h44,
    ADDR_KEEP_DATA_14             = 7'h48,
    ADDR_KEEP_DATA_15             = 7'h4c,
    ADDR_KEEP_CTRL                = 7'h50,
    ADDR_TOTAL_CAPTURESIZE_DATA_0 = 7'h54,
    ADDR_TOTAL_CAPTURESIZE_DATA_1 = 7'h58,
    ADDR_TOTAL_CAPTURESIZE_CTRL   = 7'h5c,
    ADDR_CAPTURESIZE_DATA_0       = 7'h60,
    ADDR_CAPTURESIZE_CTRL         = 7'h64,
    ADDR_OUT_R_DATA_0             = 7'h68,
    ADDR_OUT_R_DATA_1             = 7'h6c,
    ADDR_OUT_R_CTRL               = 7'h70,
    WRIDLE                        = 2'd0,
    WRDATA                        = 2'd1,
    WRRESP                        = 2'd2,
    WRRESET                       = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_KEEP_DATA_8: begin
                    rdata <= int_keep[287:256];
                end
                ADDR_KEEP_DATA_9: begin
                    rdata <= int_keep[319:288];
                end
                ADDR_KEEP_DATA_10: begin
                    rdata <= int_keep[351:320];
                end
                ADDR_KEEP_DATA_11: begin
                    rdata <= int_keep[383:352];
                end
                ADDR_KEEP_DATA_12: begin
                    rdata <= int_keep[415:384];
                end
                ADDR_KEEP_DATA_13: begin
                    rdata <= int_keep[447:416];
                end
                ADDR_KEEP_DATA_14: begin
                    rdata <= int_keep[479:448];
                end
                ADDR_KEEP_DATA_15: begin
                    rdata <= int_keep[511:480];
                end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_KEEP_DATA_8)
            int_keep[287:256] <= (WDATA[31:0] & wmask) | (int_keep[287:256] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_KEEP_DATA_9)
            int_keep[319:288] <= (WDATA[31:0] & wmask) | (int_keep[319:288] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_KEEP_DATA_10)
            int_keep[351:320] <= (WDATA[31:0] & wmask) | (int_keep[351:320] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_KEEP_DATA_11)
            int_keep[383:352] <= (WDATA[31:0] & wmask) | (int_keep[383:352] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_KEEP_DATA_12)
            int_keep[415:384] <= (WDATA[31:0] & wmask) | (int_keep[415:384] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_KEEP_DATA_13)
            int_keep[447:416] <= (WDATA[31:0] & wmask) | (int_keep[447:416] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_KEEP_DATA_14)
            int_keep[479:448] <= (WDATA[31:0] & wmask) | (int_keep[479:448] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_KEEP_DATA_15)
            int_keep[511:480] <= (WDATA[31:0] & wmask) | (int_keep[511:480] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_TOTAL_CAPTURESIZE_DATA_0)
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_TOTAL_CAPTURESIZE_DATA_1)
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_CAPTURESIZE_DATA_0)
