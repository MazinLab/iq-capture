...
#(parameter
    ...
    output wire [63:0]                   total_capturesize,
    output wire [31:0]                   capturesize,
...
//        bit 31~0 - total_capturesize[63:32] (Read/Write)
// 0x3c : reserved
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_total_capturesize = 'b0;
    reg  [31:0]                   int_capturesize = 'b0;
        ...
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_TOTAL_CAPTURESIZE_DATA_1: begin
                    rdata <= int_total_capturesize[63:32];
                end
                ADDR_CAPTURESIZE_DATA_0: begin
                    rdata <= int_capturesize[31:0];
                end
...
// int_total_capturesize[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_total_capturesize[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_TOTAL_CAPTURESIZE_DATA_1)
            int_total_capturesize[63:32] <= (WDATA[31:0] & wmask) | (int_total_capturesize[63:32] & ~wmask);
    end
end

// int_capturesize[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_capturesize[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CAPTURESIZE_DATA_0)
            int_capturesize[31:0] <= (WDATA[31:0] & wmask) | (int_capturesize[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_IQOUT_DATA_0)
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_IQOUT_DATA_1)
