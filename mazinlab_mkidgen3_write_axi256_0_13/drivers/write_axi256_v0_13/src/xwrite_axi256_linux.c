...
typedef struct {
    ...
    XWrite_axi256_uio_map maps[ MAX_UIO_MAPS ];
} XWrite_axi256_uio_info;

/***************** Variable Definitions **************************************/
static XWrite_axi256_uio_info uio_info;
...
static int uio_info_read_name(XWrite_axi256_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XWrite_axi256_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XWrite_axi256_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XWrite_axi256_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XWrite_axi256_Initialize(XWrite_axi256 *InstancePtr, const char* InstanceName) {
	XWrite_axi256_uio_info *InfoPtr = &uio_info;
